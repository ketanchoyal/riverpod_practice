...
class _ViewModelBuilderState<T extends ChangeNotifier>
  ...
  T? _viewModel;
  late var _changeNotifierProvider;
  ...
  void _createViewModel() {
    if (widget.disposeViewModel)
      _changeNotifierProvider =
          AutoDisposeChangeNotifierProvider((_) => widget.viewModelBuilder());
    else
      _changeNotifierProvider =
          ChangeNotifierProvider((_) => widget.viewModelBuilder());
    _viewModel = context.read(_changeNotifierProvider);
  ...
  Widget build(BuildContext context) {
    if (widget.providerType == _ViewModelBuilderType.NonReactive) {
      ...
      //     var viewModel = watch(_changeNotifierProvider);
      //     return widget.builder(context, _viewModel!, widget.staticChild);
      //   },
      // );

      return ProviderListener<T>(
        onChange: (context, model) {},
        provider: _changeNotifierProvider,
        child: widget.builder(context, _viewModel!, widget.staticChild),
      );
    }
    ...
    return Consumer(
      builder: (context, watch, child) {
        ...
        return ProviderListener<T>(
          onChange: (context, model) {},
          provider: _changeNotifierProvider,
          child: builderWithDynamicSourceInitialise(context, viewModel, child),
        );
      },
    );
